...
Данный генератор не занимается сам установкой перехватов. Для этого можно использовать любую из существующих библиотек.
Текущая реализация тестировалась с библиотекой [Detours](https://github.com/microsoft/detours/wiki).
...
Прототипы функций должны быть каждый в отдельной строке, перенос прототипа на несколько строк не поддерживается.
...
`types` - выводит типы, обнаруженные в возвращаемых значениях, а также типы, обнаруженные в параметрах функций-прототипов.
Может использоваться для обновления файла начальных имен параметров INITIAL_NAMES_INI; текстовый отчет.

`config` - файл с define'ами основных опций генерации; исходник на C++.
...
`def` - генерирует .DEF файл для создания прокси-DLL; текстовый DEF-файл.
...
Запускаем `umba-dll-proxy-gen`, при этом файлы INITIAL_NAMES_INI и PROXY_GENERATION_CONFIG_INI на данном этапе могут быть пустыми. Задаём генерацию 
...
После произведённых действий получаются исходные тексты готового для компиляции модуля прокси/хук-DLL.

Для того, чтобы добавить своё поведение для перехватчиков, следует определить соответствующие макросы с реализацией кастомного перехвата (см. макрос KERNEL32_HOOK_HANDLE_CREATEFILE2 вы листинге выше).
